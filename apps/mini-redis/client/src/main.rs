//!
//! Start a server(port 6379) in one terminal
...
async fn main() -> mini_redis::Result<()> {
    ...
    println!("ping: ");
    {
        let msg = Bytes::from("world");
        let value = client.ping(Some(msg)).await?;
    ...
    println!("\npublish: topic ");
    {
        let channel = String::from("topic");
        let message = Bytes::from("event");
    ...
    println!("\nsubscribe: topic ");
    {
        let channels = vec![String::from("topic")];
